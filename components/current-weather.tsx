...
export const CurrentWeather = ({ data, city }: { data: List; city: City }) => {
  ...
  const { main, id } = current;
  const { temp, temp_max, temp_min } = data.main;
  const t = useTranslations("CurrentWeather");
  return (
    <Card>
      <CardHeader>
        <div className="flex justify-between text-lg font-semibold">
          <span>{t(getDayName(city.timezone, data.dt, "long"))}</span>
        ...
        <div className="flex flex-1 justify-between items-end">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="opacity-70 text-sm">{t(id.toString())}</span>
            </div>
          </div>
